...
float const2 = 0.35 * 2.35;
float const3 = 0.33 * 2.5;

float rampFactor = 0.0; 
float rampIncrement = 0.01; 
...
float targetPoints[6] = { 10,10,20,10,20,20}; 
...
void loop() {
  ...
  while(millis() < T_const*timer){
    float targetX = targetPoints[currentPointIndex];
    float targetY = targetPoints[currentPointIndex + 1];
    ...
    currentX = targetPoints[currentPointIndex];
    currentY = targetPoints[currentPointIndex + 1];
